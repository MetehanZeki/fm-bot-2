...
const ayarlar = require('./ayarlar.json');//Fırat & Mete
const chalk = require('chalk');
const moment = require('moment');
var Jimp = require('jimp');//Fırat & Mete
const { Client, Util } = require('discord.js');
const weather = require('weather-js')//Fırat & Mete//Fırat & Mete
const fs = require('fs');
const db = require('quick.db');//Fırat & Mete
require('./util/eventLoader.js')(client);
const path = require('path');//Fırat & Mete
...
const Hserver = new Discord.WebhookClient("735028957640130560", "Uo4HxYbrsKKO0OrB-Hh5y3dXp39TJPnMNl2WPPH7s_vcHodK89PxrDwGuFwpqkle_ZMK")//Video Duyuruları

const app = express();
app.get("/", (request, response) => {
  console.log(Date.now() + " Fırat & Mete | Youtube Channel");
...
// client.on('debug', e => {
//   Fırat & Mete(chalk.bgBlue.green(e.replace(regTokenfırat&mete 'that was redacted')));
// }); //Fırat & Mete//
...
client.on('guildMemberAdd', async member => {
  await member.addRole(`671292900004593675`) //id yazan yere verilecek rol misafir (kayıtsız)
...
takizaman = `✅ Güvenli`}require("moment-duration-format");
 ...
 let message = member.guild.channels.find(x => x.id === `671293294319239168`).send(`**Fırat & Mete Ailesine Hoş Geldin!**\n \n**<a:atesli_kral:732601532700426310> **Sunucumuza Hoş Geldin **${member}** Seninle Beraber **${member.guild.memberCount}** Kişiyiz **\n \n ** <a:bolunen_elmas:732601549934690368> ** Kayıt İçin Ses Teyit Odalarına Girip Gerçek İsmini ve Yaşını Söylemen Lazım!**\n \n ** <a:renkli_ok:732601512932409435> ** <@&739754853936660540> ** Yetkisine Sahip Kişiler Seninle İlgilenecekler.** \n \n ** <a:elmas:732601532079407186> **Sunucumuzun Sınırsız Davet Bağlantısı:** https://discord.gg/kNC9dxE**\n \n ** <a:yildiz:732601517533823097> Hesap: ** ${new Date().getTime() - member.user.createdAt.getTime() < 45*24*60*60*1000 ? " ``❌ Tehlikeli``!" : "``✔️ Güvenli``!"}\n \n**<a:hypequad:732601510805897266> Bot Yapımcıları : <@!424492496899538944> ve <@!444410331423309824>**`,) 
...
if (msg.channel.type === "dm") return;
       return client.users.find(ayarlar.sahip).send(`Sunucu'ya Vuruyorlar \nSunucu Bölgesini Değiştirdim \n __**${yenibölge}**__  __**Sunucu Pingimiz**__ :`+ client.ping)
           ...
           .then(g => sChannel.channel.send(" **Fırat & Mete** sunucunun bölgesi "+ g.region  + " olarak değiştirildi")).then(tlhamert => tlhamert.delete(5000))
           .catch(console.error);
}});
...
client.on("message", async msg => {
  if (msg.content.toLowerCase() === 'sa') {
    msg.reply('Ve Aleyküm Selam İyi Eğlenceler! ^^');
  }
});
//------------------------------------------------------------------\\

client.on("message", msg => {
var dm = client.channels.cache.get("741940346409451610") //Gönderilecek Kanal İD
...
client.on('ready', ()=>{
client.channels.get('741941426665029683').join()
})
...
client.on("guildMemberAdd", message => {
  client.channels.get("741945358795669564").setName(`Toplam Üye • ${message.guild.memberCount} `);
...
client.on("guildMemberAdd", async member => {
  if (member.guild.id !== "658332887078404097") return;
  let channel = client.channels.get("742012130454143026");
...
client.on("message", m => {
  if (m.channel.id !== "658352334799241217") { //buraya o kanalın ID'si yazılacak.
    return;
  }
